...
export const CartDrawer: React.FC<CartDrawerProps> = ({
  ...
  const formatWhatsAppOrderText = (order: Order) => {
    ...
    (order.items || []).forEach((it) => {
      const itemTotal = (it.product.price * it.quantity).toFixed(2).replace('.', ',');
      msg += `• ${it.quantity}x ${it.product.name}${it.product.tamanhos ? ` (${it.product.tamanhos})` : ''} - R$ ${itemTotal}\n`;
    });
                ...
                {cartItems.length > 0 ? (
                      ...
                      <div key={(item.product?.id || Math.random().toString())} className="pt-4 first:pt-0 flex gap-3.5">
                        ...
                        <div className="flex-1 min-w-0">
                          ...
                          <p className="text-xs font-black text-amber-300 mt-1">
                            R$ {Number(item.product?.price || 0).toFixed(2).replace('.', ',')}
                          </p>
                  ...
                  <div className="h-full flex flex-col items-center justify-center text-center p-6 space-y-3">
                    ...
                    <p className="text-xs text-purple-300/80 max-w-xs">
                      Escolha meias divertidas ou camisetas do Seu José e volte aqui para finalizar.
                    </p>
                  </div>
                )}
              </div>
